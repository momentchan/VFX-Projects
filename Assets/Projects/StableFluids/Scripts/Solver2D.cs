...
namespace StableFluid {
    ...
    public class Solver2D : SolverBase {
        ...
        protected override void VelocityStep() {
            ...
            computeShader.Dispatch(kernelMap[ComputeKernels.ProjectStep3], Mathf.CeilToInt((float)velocityTex.width / gpuThreads.x), Mathf.CeilToInt((float)velocityTex.height / gpuThreads.y), 1);
            ...
            computeShader.Dispatch(kernelMap[ComputeKernels.AdvectVelocity], Mathf.CeilToInt((float)velocityTex.width / gpuThreads.x), Mathf.CeilToInt((float)velocityTex.height / gpuThreads.y), 1);
